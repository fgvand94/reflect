...
const cookieIndex = req.headers.cookie.indexOf('sessionid');
    ...
    //and sending or viewing personal messages. 
        ...
        pool.query(`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 11, cookieIndex + 139)}'`, (e, re) => {
            if (e) {
                console.log(e);
            };
            console.log(re.rows);
        ...
        pool.query(`select * from pictures where username = '${req.url.slice(6)}'`, (err, response) => {
          ...
          console.log(obj.photos[0]);
            ...
            (
                ...
                , a.count, posts.id as postsid, conversation.id as conversationid, posts.convid, conversation.user1name, conversation.user2name,
...
app.post('/new-conversation', (req, res) => {
    ...
    where users.name = '${req.body.user}' order by conversations.conversationid desc limit 1`, (err, resp) => {
        ...
        if (resp.rows[0].name) {
            ...
            pool.query (`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 11, cookieIndex + 139)}'`, (er, re) => {
