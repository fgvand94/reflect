...
app.get('/', (req, res) => {
    ...
    let obj = {
        ...
        pool.query(`select name from users where session = '${cookie}'`, (err, resp) => {
...
app.get('/user-*', (req, res) => {
  console.log(req.headers.cookie);
    ...
    //and sending or viewing personal messages. 
        ...
        pool.query(`select name from users where session = '${cookie}'`, (e, re) => {
            if (e) {
                console.log(e);
            };
            console.log(re.rows);
...
app.post('/new-conversation', (req, res) => {
    ...
    where users.name = '${req.body.user}' order by conversations.conversationid desc limit 1`, (err, resp) => {
        ...
        if (resp.rows[0].name) {
            ...
            pool.query (`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 138)}'`, (er, re) => {
...
app.post('/conversation-add', (req, res) => {
    ...
    pool.query(`select id from conversationposts order by id desc limit 1`, (err, resp) => {
        ...
        pool.query (`select name from users where session = '${cookie}'`, (er, re) => {
            ...
            if (req.url.slice(req.url.lastIndexOf('_') + 3) > 0 && req.url.slice(req.url.lastIndexOf('_') + 3) <= pageCount) {
                ...
                function queryLoop () {
                    ...
                    if (i === resp.rows.length - 1 ) {
                        ...
                        pool.query(`select name from users where session = '${cookie}'`, (erro, respo) => {
...
app.get('/forums/([^/]+)/([^/]+)', (req, res) => {
    ...
    let underscore = req.url.indexOf('pg');
