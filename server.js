...
app.get('/', (req, res) => {
    ...
    //username based on that sessionID and sets person to that user and isLoggedIn to true
        ...
        pool.query(`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 139)}'`, (err, resp) => {
...
const cookieIndex = req.headers.cookie.indexOf('sessionId');
    ...
    //and sending or viewing personal messages. 
        ...
        pool.query(`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 139)}'`, (e, re) => {
            if (e) {
                console.log(e);
            };
            console.log(re.rows);
        ...
        pool.query(`select * from pictures where username = '${req.url.slice(6)}'`, (err, response) => {
          ...
          console.log(obj.photos[0]);
            ...
            (
                ...
                , a.count, posts.id as postid, conversation.id as conversationid, posts.convid, conversation.user1name, conversation.user2name,
                ...
                where posts.convid = conversation.id and person.name = conversation.user1name and a.convid = posts.convid	
            ) as t
...
app.post('/new-conversation', (req, res) => {
    ...
    where users.name = '${req.body.user}' order by conversations.conversationid desc limit 1`, (err, resp) => {
        ...
        if (resp.rows[0].name) {
            ...
            pool.query (`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 139)}'`, (er, re) => {
...
app.get('/conversation/*', (req, res) => {
    ...
    order by conversationposts.id asc`, (err, resp) => {
        ...
        if (!req.headers.cookie) {
            ...
            pool.query(`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 139)}'`, (error, response) => {
...
app.get('/forums', (req, res) => {
    ...
    const cookieIndex = req.headers.cookie.indexOf('sessionId');
    const cookie = req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 139);
            ...
            if (req.url.slice(req.url.lastIndexOf('_') + 3) > 0 && req.url.slice(req.url.lastIndexOf('_') + 3) <= pageCount) {
                ...
                function queryLoop () {
                    ...
                    if (i === resp.rows.length - 1 ) {
                        ...
                        pool.query(`select name from users where session = '${req.headers.cookie.slice(cookieIndex + 10, cookieIndex + 139)}'`, (erro, respo) => {
